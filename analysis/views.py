...
def get_chart_data(table_name, period='day'):
    ...
    if period == 'day':
        group_sql = "date(upload_time)"
        label_sql = "date(upload_time) AS period_label"
    elif period == 'week':
...
def chart_data(request):
    table_name = request.GET.get('table', 'young_modul')
    period = request.GET.get('period', 'day')
    
    if not table_name:
        return JsonResponse({'error': 'Table not specified'}, status=400)
    
    try:
        data = get_chart_data(table_name, period)
